...
const stringUserName = 'greatlearning';
const stringPassword = 'password';
// for better usage please append live server url before html path!
const loginPath = 'login.html'; // http://127.0.0.1:5500/G1-S6_JS_GradedProject2/
const resumePath = 'resume.html'; // http://127.0.0.1:5500/G1-S6_JS_GradedProject2/ 
(() => {
    if (storedAuthInfo == "false") {
        if (window.location.href != loginPath) {
            window.location.href = loginPath;
        }
    }
    if (storedAuthInfo == "true") {
        if (window.location.href !== resumePath) {
...
//Below redirection code


// Below login Submit button manage
const form = document.getElementById("loginForm");
if (form) {
    const loginHandler = (e) => {
        e.preventDefault();
        document.forms["loginForm"]["username"].parentElement.classList.remove('has-error');
        document.forms["loginForm"]["password"].parentElement.classList.remove('has-error');
        let loginName = document.forms["loginForm"]["username"].value;
        let loginPassword = document.forms["loginForm"]["password"].value;
		let validateBox = document.getElementById('validCredentials')

        if (loginName == stringUserName && loginPassword == stringPassword) {
            localStorage.setItem("authInfo", true);
            window.location.href = "resume.html";
            return false;
        } else if (loginName == "" && loginPassword == "") {
            document.forms["loginForm"]["username"].parentElement.classList.add('has-error');
            document.forms["loginForm"]["password"].parentElement.classList.add('has-error');
        } else if (loginName == "") {
            document.forms["loginForm"]["username"].parentElement.classList.add('has-error');
        } else if (loginPassword == "") {
            document.forms["loginForm"]["password"].parentElement.classList.add('has-error');
        } else {
			validateBox.classList.remove('hide');
		}
    };
    form.addEventListener("submit", loginHandler);
}
// Below manage local storage
const localFunction = {
    init: () => {
        if (storedUserInfo == null) {
            localStorage.setItem(
                "userInfo",
                JSON.stringify({ username: stringUserName, password: stringPassword })
            );
        }
